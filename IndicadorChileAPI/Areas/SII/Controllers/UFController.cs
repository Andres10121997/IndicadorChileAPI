...
namespace IndicadorChileAPI.Areas.SII.Controllers
{
    ...
    public class UFController : ControllerBase, IStatistics
    {
                                                                   ...
                                                                   [FromQuery] SearchFilterModel SearchFilter)
        {
            try
            {
                if (CurrencyMath.MathematicalOperations(CurrencyList: await CurrencyInfo.GetValuesAsync(SearchFilter: SearchFilter, C_Url)).TryGetValue(key: Statistics, value: out float Value))
                {
