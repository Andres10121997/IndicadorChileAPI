...
1. Crea o asegúrate de tener un Dockerfile: Este archivo contendrá todas las instrucciones para construir tu imagen.
2. Sube el Dockerfile y tu código: Si aún no lo has hecho, sube el Dockerfile y los archivos de tu aplicación a tu repositorio de GitHub. 

## Configura las credenciales de seguridad
1. Crea un Personal Access Token (PAT): Ve a la configuración de desarrollador de tu cuenta de GitHub, crea un nuevo token con permisos para write:packages y read:packages.
...
2. Define el evento de activación: Configura el flujo para que se ejecute automáticamente cada vez que se empuje o se envíe un pull request a la rama principal (por ejemplo, on: push: branches: main).
3. Añade un trabajo (job): Este trabajo contendrá los pasos para construir y enviar la imagen.

## Escribe el código del flujo YAML
Código
