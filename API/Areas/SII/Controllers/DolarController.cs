...
namespace API.Areas.SII.Controllers
{
    ...
    public class DolarController : BaseController, IStatistics
    {
                                                                  ...
                                                                  [FromQuery] SearchFilterModel SearchFilter)
        {
            try
            {
                if (Utils.MathematicalOperations(CurrencyList: await CurrencyInfo.GetCurrenciesAsync(SearchFilter: SearchFilter, Url: this.URL)).TryGetValue(key: Statistics, value: out float Value))
                {
